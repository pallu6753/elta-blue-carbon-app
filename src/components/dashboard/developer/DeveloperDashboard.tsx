...
const widgets = [
    ...
    { title: 'MRV Reports Submitted', value: '4', icon: FileText, subtext: '3 pending verification', valueColor: 'text-yellow-500' },
  ...
  return (
    <div className="space-y-8">
        ...
        <Card className="shadow-lg">
            ...
            <CardContent>
                <CreditIssuanceChart />
                 <p className="text-xs text-muted-foreground mt-2 text-center">Total carbon credits issued per month (Simulated)</p>
            </CardContent>
        </Card>
                        ...
                        {recentProjects.map(project => (
                            <TableRow key={project.name}>
                                ...
                                <TableCell className="text-right">
                                    <Badge variant={project.status === 'Active' ? 'default' : 'secondary' } className="bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-300">{project.status}</Badge>
                                </TableCell>
                            </TableRow>
                        ))}
                    </TableBody>
